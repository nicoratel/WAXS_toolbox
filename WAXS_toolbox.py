...
from gui import Ui_WAXS_toolbox
...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.actionconvert_xyz_to_pdb.triggered.connect(self.convertxyz2pdb)      
        ...
        self.ui.qdamp_lineEdit.setText('0.014')
        self.ui.qbroad_lineEdit.setText('0.041')
    ...
    def convertxyz2pdb(self):
        try:
            import pandas as pd
            xyzfilepath=QtWidgets.QFileDialog.getOpenFileName( self,"Open xyz file",self.path,'*.xyz')
            #print(xyzfilepath)
            xyz = pd.read_csv(xyzfilepath[0], sep='\s+', skiprows=2, names=["atom", "x", "y", "z"])
            #print(xyz.head())
            ...
            pdb_filename = os.path.join(os.path.dirname(xyzfilepath[0]), os.path.basename(xyzfilepath[0]).replace("xyz", "pdb"))
            #print(output_filename)
            with open(pdb_filename, "w") as f:
                f.writelines(pdb_lines)
            print("pdb file created:", pdb_filename)
        except:
            self.ui.Console_textEdit.insertPlainText("An unidentified error occured.\n")
    ...
    def pdfcalc(self):
        ...
        ax1.set_xlabel('q  1/($\mathrm{\AA}$)',fontsize=14)
        ax1.set_ylabel('Normalized I(q)',fontsize=14)        
        
        ax2.plot(r,g_tot,'-b') 
        ax2.set_xlabel('r ($\mathrm{\AA}$)',fontsize=14)
    ...
    def open_exp_pdf(self):
        ...
        try:
            f=open(self.exp_pdf,'r')
            line=f.readline()
            while line:
               
                line=f.readline()
                if "qmin" in line:
                    qmin=str(line.split(' = ')[1])
                    self.ui.qmin_lineEdit.setText(qmin)
                if "qmax" in line:
                    qmax=str(line.split(' = ')[1])
                    self.ui.qmax_lineEdit.setText(qmax)                
            f.close()
        except:
            pass
        return self.exp_pdf
